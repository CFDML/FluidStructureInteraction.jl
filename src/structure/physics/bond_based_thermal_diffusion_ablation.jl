struct BBTAMaterial{Correction} <: Peridynamics.AbstractBondSystemMaterial{Correction} end

BBTAMaterial() = BBTAMaterial{NoCorrection}()
...
@inline function Peridynamics.allowed_material_kwargs(::BBTAMaterial)
    return (Peridynamics.DEFAULT_POINT_KWARGS..., :kc, :aph, :cv, :rft, :dx, :h, :hσ, :hϵ, :tem∞, :D)
end

function BBTAPointParameters(::BBTAMaterial, p::Dict{Symbol,Any})
    δ = Peridynamics.get_horizon(p)
    rho = Peridynamics.get_density(p)
    if haskey(p, :nu)
        msg = "keyword `nu` is not allowed for BBTAMaterial!\n"
        msg *= "Bond-based peridynamics has a limitation on the possion ratio.\n"
        msg *= "Therefore, when using BBTAMaterial, `nu` is hardcoded as 1/3 for 2D && 1/4 for 3D.\n"
    ...
    kc, aph, cv, rft, dx, h, hσ, hϵ, tem∞ = get_thermal_params(p, δ)

    if haskey(p, :D) #2D 
        bc = 9 * E / (π * dx * δ^3) # bond constant
        kp = 6 * kc / (π * dx * δ^3) # microcndicitvity constant
    else #3D
        bc = 12 * E / (π * δ^4) 
        kp = 6 * kc / (π * δ^4) 
    end   
    return BBTAPointParameters(δ, rho, E, nu, G, K, λ, μ, Gc, εc, bc, kc, kp, aph, cv, rft, dx, h, hσ, hϵ, tem∞)
end
...
function get_thermal_params(p::Dict{Symbol,Any}, δ)
    ...
    haskey(p, :dx) || throw(UndefKeywordError(:dx))   
    haskey(p, :h) || throw(UndefKeywordError(:dx))  
    ...
    dx::Float64 = float(p[:dx])
    dx ≤ 0 && throw(ArgumentError("`dx` should be larger than zero!\n"))
    h::Float64 = float(p[:h])
    ...
    return kc, aph, cv, rft, dx, h, hσ, hϵ, tem∞
end
...
# BBTA&therm store
struct BBTAthermtStorage <: Peridynamics.AbstractStorage
    ...
    ablation_deep::Vector{Float64}
    exist::Vector{Int}
end

function BBTAthermtStorage(::BBTAMaterial, ::ThermAbstep,
    ...
    ablation_deep = zeros(n_loc_points)
    exist = ones(Int, n_loc_points+n_halo)
    s = BBTAthermtStorage(position, displacement, velocity, velocity_half, acceleration,
                        b_int, b_ext, damage, bond_active, n_active_bonds,
                        temperature, pflux, hsource, ablation_deep, exist)
    return s
end

@Peridynamics.storage BBTAMaterial ThermAbstep BBTAthermtStorage

Peridynamics.point_data_field(s::BBTAthermtStorage, ::Val{:temperature}) = getfield(s, :temperature)
Peridynamics.point_data_field(s::BBTAthermtStorage, ::Val{:pflux}) = getfield(s, :pflux)
Peridynamics.point_data_field(s::BBTAthermtStorage, ::Val{:hsource}) = getfield(s, :hsource)
Peridynamics.point_data_field(s::BBTAthermtStorage, ::Val{:ablation_deep}) = getfield(s, :ablation_deep)
Peridynamics.point_data_field(s::BBTAthermtStorage, ::Val{:exist}) = getfield(s, :exist)

@Peridynamics.loc_to_halo_fields BBTAthermtStorage :temperature :exist

const BBTAStorage = Union{BBTAthermtStorage}
...
@inline function update_temperature!(b::Peridynamics.AbstractBodyChunk, Δt::Float64)
    for point_id in Peridynamics.each_point_idx(b)
        ...
        k = Δt / (param.rho * param.cv) * b.storage.exist[point_id]
...
function calc_pflux!(chunk::Peridynamics.AbstractBodyChunk, mbd_v::Vector{Float64}, mbd_t::Vector{Float64})
    ...
    for point_id in eachindex(chunk.system.chunk_handler.loc_points)
        pflux_point!(storage, system, mat, paramsetup, point_id, mbd_v, mbd_t)
    end
    return nothing
end

function pflux_point!(storage::BBTAStorage, system::Peridynamics.BondSystem, 
    ::BBTAMaterial, param::BBTAPointParameters, i::Int, mbd_v::Vector{Float64}, mbd_t::Vector{Float64}) 

    for bond_id in system.bond_ids[i]
        bond = system.bonds[bond_id]
        j, L = bond.neighbor, bond.length
        
        volm, mof_th = mbd_v[bond_id], mbd_t[bond_id]
        
        Δtem = storage.temperature[1, j] - storage.temperature[1, i]
        storage.pflux[1, i] += storage.bond_active[bond_id] * (param.kp * Δtem / L) * 
        system.volume[j] * volm * mof_th * storage.exist[i] * storage.exist[j]
    end
    return nothing
end
