...
struct Xjob{S<:Peridynamics.AbstractSpatialSetup, FT<:AbstractFlowTimeSolver, ST<:Peridynamics.AbstractTimeSolver, O<:Peridynamics.AbstractJobOptions}
    ...
    function Xjob(flow_setup::KitBase.SolverSet, spatial_setup::S, f_time_solver::FT, s_time_solver::ST, options::O) where {S, FT, ST, O}
...
function Xjob(flow_setup::KitBase.SolverSet, spatial_setup::S, f_time_solver::FT, s_time_solver::ST; kwargs...) where {S, FT, ST}
    o = Dict{Symbol,Any}(kwargs)
    Peridynamics.check_kwargs(o, JOB_KWARGS)
    options = fsi_get_job_options(spatial_setup, s_time_solver, o)
    return Xjob(flow_setup, spatial_setup, f_time_solver, s_time_solver, options)
end

function Base.show(io::IO, @nospecialize(job::Xjob))
...
function Base.show(io::IO, ::MIME"text/plain", @nospecialize(job::Xjob))
